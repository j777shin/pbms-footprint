...
def load_image(path: str):
    ...
    :return:
    """
    # Use IMREAD_UNCHANGED to preserve 4-channel images (RGB + mask)
    ...
    if img.ndim == 2:
        ...
        if img.shape[2] == 3:
            # 3-channel BGR: convert to RGB
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        elif img.shape[2] == 4:
            # 4-channel: assume RGBA or RGB+mask format
            # cv2.imread with IMREAD_UNCHANGED preserves RGBA as-is
            # Our 4-channel format is RGB + mask (saved via np.dstack)
            # Keep as-is (channels are R, G, B, mask)
            pass
        else:
